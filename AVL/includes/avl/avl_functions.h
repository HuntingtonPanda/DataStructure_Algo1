...
bool tree_erase(tree_node<T>*& root, const T& target) {
    ...
    if (target < root->_item) {
        ...
        if (!root->_left || !root->_right) {
            ...
            root->_item = temp->_item; // Copy inorder successor's value
            tree_erase(root->_right, temp->_item); // Delete the inorder successor
        }
    }
