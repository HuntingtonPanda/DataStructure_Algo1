...
public:
    ...
    bool contains(const T &target, TT& result)const{
        if(tree_search(_root, target)){
            result = tree_search(_root, target)->_item;
        }
...
AVL<T>& AVL<T>::operator=(const AVL<T> &rhs){
    if (this != &rhs) {
        tree_clear(_root);
        this->_root = tree_copy(rhs._root);
    }
    return *this;
}
